...
export default function Create() {
    const { specializations } = usePage<{ specializations: string[] }>().props;
    ...
    function handleSubmit(e: React.FormEvent) {
        e.preventDefault();
        setProcessing(true);
        router.post(contractors.store().url, { ...form, ...files }, {
            preserveState: true,
            preserveScroll: true,
            onSuccess: () => {
    ...
    return (
        <>
            <Head title="Create Contractor" />
            <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
                <div className="flex items-center gap-4">
                    <Link href={contractors.index().url}>
                        <Button variant="ghost" size="icon"><ArrowLeft className="h-4 w-4" /></Button>
                    </Link>
                    <h1 className="text-2xl font-bold">Create Contractor</h1>
                </div>
...
Create.layout = {
    breadcrumbs: [
        ...
        { title: 'Create', href: contractors.create().url },
    ],
};
