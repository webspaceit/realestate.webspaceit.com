...
export default function Create({ buildings, projects, bulkUnits }: { buildings: { id: number; name: string; project_id: number | null }[]; projects: { id: number; name: string }[]; bulkUnits: { id: number; unit_number: string; unit_name: string | null }[] }) {
    ...
    const toggleBulkUnit = (id: number, unitNumber: string, unitName: string | null) => {
        setSelectedIds((prev) => {
            let next: number[];
            if (prev.includes(id)) {
            ...
            const names = selected.map((u) => u.unit_name || u.unit_number).join(', ');
    ...
    return (
        <>
            ...
            <div className="flex items-center gap-2 mb-6">
                ...
                <h1 className="text-2xl font-semibold">Create Flat</h1>
            </div>
                ...
                <CardContent>
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="space-y-2">
                                        ...
                                        {bulkUnits.map((u) => (
                                            <label key={u.id} className="flex items-center gap-2 rounded px-2 py-1 text-sm hover:bg-muted cursor-pointer">
                                                <Checkbox
                                                    ...
                                                    onCheckedChange={() => toggleBulkUnit(u.id, u.unit_number, u.unit_name)}
                                                />
                                ...
                                {bulkUnits.length === 0 && (
                                    <p className="text-muted-foreground text-xs">No bulk units available. Create bulk units first.</p>
                                )}
                            </div>
                            <div className="space-y-2">
...
Create.layout = {
    breadcrumbs: [
        ...
        { title: 'Create', href: flats.create() },
    ],
};
